...
def get_profile(username: str) -> dict:
    all_p = load_all_profiles()
    # Case-insensitive lookup to avoid creating duplicate profiles
    matched_key = next((k for k in all_p if k.lower() == username.lower()), None)
    if not matched_key:
    ...
    return all_p[matched_key]


def save_profile(username: str, profile: dict):
    all_p = load_all_profiles()
    # Use the matched key to avoid key mismatch
    matched_key = next((k for k in all_p if k.lower() == username.lower()), username)
    # Preserve fields that are managed separately (quest_history, streaks)
    existing = all_p.get(matched_key, {})
    if "quest_history" not in profile:
        profile["quest_history"] = existing.get("quest_history", [])
    if "streaks" not in profile:
        profile["streaks"] = existing.get("streaks", {})
    if "password_hash" not in profile:
        profile["password_hash"] = existing.get("password_hash", "")
    if "password_hint" not in profile:
        profile["password_hint"] = existing.get("password_hint", "")
    if "email" not in profile:
        profile["email"] = existing.get("email", "")
    all_p[matched_key] = profile
    save_all_profiles(all_p)
